...
**Two-stage** triage before any specialist runs. Every email first passes a relevance check, then a sensitivity check. Only emails that clear both ever reach a specialist. This keeps risk low: nothing gets an automated reply unless it's confirmed relevant and confirmed safe.
...
Python 3.12 · LangChain / LangGraph · Azure AI Foundry (GPT-5-mini for triage and specialists, text-embedding-3-small) · Pinecone · PostgreSQL + SQLAlchemy + Alembic · Gmail API · FastAPI · Docker · GitHub Actions · Azure Container Apps
...
One Docker image serves both roles, selected at runtime by the SERVICE_MODE environment variable: set to dashboard, the image runs the FastAPI dashboard via uvicorn; otherwise, it runs the poller. The poller runs as a scheduled Azure Container Apps Job (every 5 minutes); the dashboard runs as an always-on Azure Container App. Postgres runs as its own internal-only Container App in the same environment — reachable only by the dashboard and poller, never the public internet. CI builds and pushes the image to ghcr.io on every push to main.

## Operational notes
Processes at most 25 unread emails per poll cycle; anything beyond that is picked up on the next run.
RAG conversation memory is capped at the last 5 exchanges per email thread to keep prompts bounded.
Overlapping poll cycles (for example, a manual trigger running close to the scheduled one) can occasionally produce a duplicate reply on the same email, since exactly-once delivery across Gmail, Postgres, and LangGraph checkpointing isn't guaranteed.
Postgres runs on ephemeral container storage rather than a persistent volume — a container restart clears all data and requires re-running migrations.
