...
- Processes at most 25 unread emails per poll cycle.
- RAG conversation memory is capped at the last 5 exchanges per thread.
- No exactly-once delivery guarantee across Gmail, Postgres, and LangGraph checkpointing — a failure between the database write and the Gmail reply can, rarely, cause a duplicate reply or a re-proposed order/ticket summary.
- Deployed Postgres uses ephemeral container storage rather than a persistent volume — a deliberate simplification since this system is meant to be deployed, demoed, and torn down.
